...
export default function TrainingModule() {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-24 text-gray-500">
        <Loader2 className="animate-spin h-8 w-8 mr-3" style={{ color: ACCENT_INK }} /> Clubs laden…
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-black text-gray-900">Voetballessen</h1>
      ...
      <div className="space-y-2">
        ...
        {clubs.map(club => {
          ...
          return (
            <div key={club.id} className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
              <button
                onClick={() => setSelected(open ? null : club.id)}
                className="w-full flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition-colors text-left"
              >
                <Building2 size={16} className="text-gray-500 shrink-0" />
                <span className="flex-1 font-semibold text-gray-900 text-sm">{club.name}</span>
                <span
                  className="text-[10px] font-black px-2 py-0.5 rounded-full shrink-0"
                  style={club.subscription_tier === 'pro'
                    ? { backgroundColor: `${ACCENT_INK}14`, color: ACCENT_INK }
