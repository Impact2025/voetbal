...
function FAQItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div className={`bg-white border rounded-xl overflow-hidden transition-colors shadow-sm ${open ? 'border-neon/40' : 'border-slate-200'}`}>
      <button
      ...
      >
        ...
        <ChevronDown
          size={18}
          className={`text-neon-ink flex-shrink-0 transition-transform duration-300 ${open ? 'rotate-180' : ''}`}
        />
      </button>
      <div
    ...
    <section id="faq" className="py-24 bg-slate-50">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12 space-y-3">
          <p className="text-neon-ink text-sm font-semibold uppercase tracking-widest">FAQ</p>
          <h2 className="text-3xl sm:text-4xl font-black">Veelgestelde vragen</h2>
          <p className="text-slate-500">
            Geen antwoord op jouw vraag?{' '}
            <a href="mailto:info@skillkaart.nl" className="text-neon-ink hover:underline">
              Mail ons direct
            </a>
            .
          </p>
